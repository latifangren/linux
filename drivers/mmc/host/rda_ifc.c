		      ...
		      u32 xfer_size, HAL_IFC_MODE_T ifc_mode)
{
	...
	if (rda_soc_is_older_metal10()) {
		u32 mem_u32 = (u32)(uintptr_t)mem_addr;

		if (!IS_ALIGNED(mem_u32, 16) &&
		    ((mem_u32 >> PAGE_SHIFT) != ((mem_u32 + xfer_size - 1) >> PAGE_SHIFT))) {
			pr_err("IFC cross page boundary: mem_addr = %p, size = %u\n",
			       mem_addr, xfer_size);
			BUG_ON(1);
		}
	}
	...
	hwp_sysIfc->std_ch[channel].sg_table[0].start_addr = (u32)(uintptr_t)mem_addr;
