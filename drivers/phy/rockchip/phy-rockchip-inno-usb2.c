...
static int rk3399_usb2phy_tuning(struct rockchip_usb2phy *rphy)
{
	...
	if (rphy->phy_cfg->reg == 0xe450) {
		/* Avoid HS handshake failures on PHY0. */
		ret |= regmap_write(rphy->grf, 0x4480, GENMASK(17, 16));
		ret |= regmap_write(rphy->grf, 0x44b4, GENMASK(17, 16));
	} else {
		/* Avoid HS handshake failures on PHY1, which feeds the FE1.1S hub. */
		ret |= regmap_write(rphy->grf, 0x4500, GENMASK(17, 16));
		ret |= regmap_write(rphy->grf, 0x4534, GENMASK(17, 16));
	}

	if (!device_property_read_bool(rphy->dev, "rockchip,u2phy-tuning"))
		return ret;

	if (rphy->phy_cfg->reg == 0xe450) {
		ret |= regmap_write(rphy->grf, 0x448c, GENMASK(23, 16) | 0xe3);
		ret |= regmap_write(rphy->grf, 0x44b0, GENMASK(18, 16) | 0x07);
		ret |= regmap_write(rphy->grf, 0x4480, GENMASK(30, 30) | 0x4000);
	} else {
		ret |= regmap_write(rphy->grf, 0x450c, GENMASK(23, 16) | 0xe3);
		ret |= regmap_write(rphy->grf, 0x4530, GENMASK(18, 16) | 0x07);
		ret |= regmap_write(rphy->grf, 0x4500, GENMASK(30, 30) | 0x4000);
	}
