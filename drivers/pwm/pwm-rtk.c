...
#define to_rtd1295_pwm_chip(chip) \
	pwmchip_priv(chip)
...
static int set_real_freq_by_target_freq(struct rtd1295_pwm_chip *pc,
					int hwpwm,
					int target_freq)
{
	...
	{
		const int min_ocd = 0;
		const int max_ocd = 255;
		const int min_csd = 0;
		const int max_csd = 15;
...
static int set_real_period_by_target_period(struct rtd1295_pwm_chip *pc,
					    int hwpwm,
					    int target_period_ns)
{
	int target_freq;
	int real_freq;
	const int base_ns = 1000000000;
	...
	return base_ns / real_freq;
}

static int set_real_freq_by_target_div(struct rtd1295_pwm_chip *pc,
				       int hwpwm,
				       int clksrc_div,
				       int clkout_div)
{
	...
	if (clksrc_div < 0 || clksrc_div > 15)
		return -EINVAL;

	if (clkout_div < 0 || clkout_div > 255)
		return -EINVAL;

	opt_div = BIT(clksrc_div + 1) * (clkout_div + 1);

	if (!opt_div)
		return -EINVAL;

	real_freq = pc->base_freq / opt_div;
...
static int set_clk_duty(struct rtd1295_pwm_chip *pc,
			int hwpwm,
			int duty_rate)
{
	int clkout_div;
	...
	pc->clk_duty[hwpwm] =
		(duty_rate * (clkout_div + 1) / 100) - 1;

	if (pc->clk_duty[hwpwm] < 0)
		pc->clk_duty[hwpwm] = 0;

	if (pc->clk_duty[hwpwm] > clkout_div)
		pc->clk_duty[hwpwm] = clkout_div;
...
static int rtd1295_pwm_apply(struct pwm_chip *chip,
			     ...
			     const struct pwm_state *state)
{
	...
	real_period_ns =
		set_real_period_by_target_period(pc,
						 pwm->hwpwm,
						 state->period);
	...
	duty_rate = div64_u64((u64)state->duty_cycle * 100,
			      real_period_ns);

	set_clk_duty(pc, pwm->hwpwm, duty_rate);

	pc->enable[pwm->hwpwm] = state->enabled ? 1 : 0;
...
static int rtd1295_pwm_get_state(struct pwm_chip *chip,
				 struct pwm_device *pwm,
				 struct pwm_state *state)
{
	struct rtd1295_pwm_chip *pc = to_rtd1295_pwm_chip(chip);

	state->enabled = pc->enable[pwm->hwpwm];
	state->period = 1000000000ULL / pc->out_freq[pwm->hwpwm];
	state->duty_cycle =
		(state->period * pc->duty_rate[pwm->hwpwm]) / 100;

	return 0;
}

static const struct pwm_ops rtd1295_pwm_ops = {
	.apply = rtd1295_pwm_apply,
...
static ssize_t pwm_store_out_freq(struct device *dev,
				  ...
				  int hwpwm)
{
	...
	if (value <= 0 || value > 27000000) {
		pr_err("%s %s ==== input should be between 1 Hz and 27 MHz\n",
		       DEV_NAME, __func__);
		return -EINVAL;
	}

	if (pc->out_freq[hwpwm] == value) {
		pr_debug("%s %s ==== hwpwm=%d, same value=%d, skip\n",
			 DEV_NAME, __func__, hwpwm, value);
		return count;
	}

	ret = set_real_freq_by_target_freq(pc, hwpwm, value);
	if (ret < 0)
		return ret;
...
static int rtd1295_pwm_probe(struct platform_device *pdev)
{
	...
	struct device_node *pwm0;
	struct device_node *pwm1;
	struct device_node *pwm2;
	struct device_node *pwm3;
	int ret;
	int i;
	u32 val;
	...
	chip = devm_pwmchip_alloc(&pdev->dev,
				  NUM_PWM,
				  sizeof(struct rtd1295_pwm_chip));
	if (IS_ERR(chip))
		return PTR_ERR(chip);

	pwm = pwmchip_priv(chip);
	...
	dev_info(&pdev->dev, "PWM driver initialized successfully\n");
	return 0;
}

static void rtd1295_pwm_remove(struct platform_device *pdev)
{
	struct rtd1295_pwm_chip *pc = platform_get_drvdata(pdev);
	int i;
