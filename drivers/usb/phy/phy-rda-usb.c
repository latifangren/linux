...
int __weak android_usb_ready(void)
{
	return 0;
}

int __weak musb_g_addressed(struct usb_gadget *gadget)
{
	(void)gadget;
	return 0;
}

int __weak musb_disconnect_gadget(struct usb_gadget *gadget)
{
	if (gadget)
...
int __weak charger_status_notify_is_ignore(void)
{
	return 0;
}
...
static void cable_type_work(struct work_struct *work)
{
	...
	if (!gadget) {
		pr_warn("the musb phy isn't configured\n");
		return;
	}
...
static int __init gpio_vbus_probe(struct platform_device *pdev)
{
	...
	if (!err) {
		...
		/* Keep delayed enable intent, but use modern IRQ status flags. */
		gpio_otg_switch_irq_enable(irq);
	}
