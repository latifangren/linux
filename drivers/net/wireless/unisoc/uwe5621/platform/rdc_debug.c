...
static int wcn_get_file_size(const char *path, int *size)
{
	struct file *fp;
	...
	*size = (int)i_size_read(file_inode(fp));
	filp_close(fp, NULL);
...
static int wcn_find_cp2_file_num(char *path, loff_t *pos)
{
	...
	if (wcn_cp2_log_cover_old) {
		for (i = 0; i < wcn_cp2_file_max_num; i++) {
			...
			ret = wcn_get_file_size(wcn_cp2_file_path, &config_size);
			if (ret)
				break;
			exist_file_num++;
...
static void wcn_config_log_file(void)
{
	...
	for (index = 0; index < WCN_DEBUG_CFG_MAX_PATH_NUM; index++) {
		ret = wcn_get_file_size(wcn_cp2_config_path[index],
					&config_size);
		if (!ret) {
