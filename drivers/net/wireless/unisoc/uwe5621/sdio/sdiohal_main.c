...
#if __has_include(<linux/amlogic/aml_gpio_consumer.h>)
#include <linux/amlogic/aml_gpio_consumer.h>
#endif

#ifndef GPIO_IRQ_LOW
#define GPIO_IRQ_LOW 0
#endif
#ifndef GPIO_IRQ_HIGH
#define GPIO_IRQ_HIGH 1
#endif

extern int wifi_irq_num(void) __attribute__((weak));
extern int wifi_irq_trigger_level(void) __attribute__((weak));
extern void sdio_reinit(void) __attribute__((weak));
extern void sdio_set_max_regs(unsigned int size) __attribute__((weak));

static inline int aml_wifi_irq_num(void)
{
	if (wifi_irq_num)
		return wifi_irq_num();
...
static inline int aml_wifi_irq_level(void)
{
	if (wifi_irq_trigger_level)
		return wifi_irq_trigger_level();

	return GPIO_IRQ_HIGH;
}

static inline void aml_sdio_reinit(void)
{
	if (sdio_reinit)
		sdio_reinit();
}

static inline void aml_sdio_set_max_regs(unsigned int size)
{
	if (sdio_set_max_regs)
		sdio_set_max_regs(size);
}
#endif
...
static int sdiohal_host_irq_init(unsigned int irq_gpio_num)
{
	...
	p_data->irq_num = aml_wifi_irq_num();
	if (p_data->irq_num < 0 && irq_gpio_num)
		p_data->irq_num = gpio_to_irq(irq_gpio_num);
	if (aml_wifi_irq_level() == GPIO_IRQ_LOW)
...
int sdiohal_scan_card(void)
{
	...
	if (WCN_CARD_EXIST(&p_data->xmit_cnt)) {
		 ...
		 */
		if (p_data->sdio_dev_host != NULL &&
		    p_data->sdio_dev_host->card && mmc_power_up)
			mmc_power_up(p_data->sdio_dev_host,
				     p_data->sdio_dev_host->card->ocr);
		/*
		 * setting sdio max request size to 512kB
		 * to improve transmission efficiency.
		 */
