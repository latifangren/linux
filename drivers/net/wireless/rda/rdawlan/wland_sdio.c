...
static unsigned long wland_last_txctl_recover_jiffies;
static unsigned long wland_last_txctl_powercycle_jiffies;
static unsigned long wland_last_rxctl_powercycle_jiffies;

static int wland_sdio_rxctl_try_read(struct wland_sdio *bus, u8 *msg,
				     uint msglen)
{
	uint rxlen = 0;
...
static int wland_sdio_rxctl_poll_after_recover(struct wland_sdio *bus,
						u8 *msg, uint msglen,
						uint timeout_ms)
{
	uint waited = 0;
	...
	while (waited < timeout_ms) {
		ret = wland_sdio_rxctl_try_read(bus, msg, msglen);
		if (ret >= 0)
			return ret;
		msleep(20);
		waited += 20;
...
static int wland_sdio_rxctl_wait_once(struct wland_sdio *bus, u8 *msg,
				      uint msglen, bool *pending,
				      int *timeleft)
{
	int ret;
...
static int wland_sdio_txctl_once(struct wland_sdio *bus,
				 ...
				 u8 *msg, uint msglen)
{
	...
	if (bus->ctrl_frame_stat) {
		WLAND_ERR("txctl wait timeout\n");
		return -ETIMEDOUT;
	}

	ret = bus->ctrl_frame_send_success ? 0 : -EIO;
	return ret;
}
...
static int wland_chip_wake_up(struct wland_sdio *bus)
{
	int ret = 0;
...
static int wland_chip_goto_sleep(struct wland_sdio *bus)
{
	int ret = 0;
 ...
 */
static struct sk_buff *wland_sdio_readframes(struct wland_sdio *bus)
{
	int ret = 0;
...
static int wland_sdio_bus_txctl(struct device *dev, u8 * msg, uint msglen)
{
	...
	ret = wland_sdio_txctl_once(bus, bus_if, msg, msglen);
	if (ret < 0 && (ret == -EIO || ret == -ETIMEDOUT) &&
	    time_after(jiffies,
		wland_last_txctl_recover_jiffies + msecs_to_jiffies(3000))) {
		wland_last_txctl_recover_jiffies = jiffies;
		WLAND_ERR("txctl failed (%d), trigger host1 recover + retry\n",
			ret);
	...
	if (ret < 0 && (ret == -EIO || ret == -ETIMEDOUT) &&
	    time_after(jiffies,
		wland_last_txctl_powercycle_jiffies + msecs_to_jiffies(15000))) {
		wland_last_txctl_powercycle_jiffies = jiffies;
		WLAND_ERR("txctl still failed (%d), power-cycle wifi + retry\n",
			ret);
		rda_wifi_power_off();
		msleep(80);
		rda_wifi_power_on();
...
static int wland_sdio_bus_rxctl(struct device *dev, u8 * msg, uint msglen)
{
	...
	ret = wland_sdio_rxctl_wait_once(bus, msg, msglen, &pending, &timeleft);
	if (ret >= 0)
		return ret;

	if (ret == -ETIMEDOUT) {
		WLAND_DBG(SDIO, DEBUG,
			"rxctl timeout, trigger host1 recover + short retry\n");
		rda_mmc_set_sdio_irq(1, true);
		rda_mmc_bus_scan(1);
		WAKE_RX_WORK(bus);
		msleep(20);
		ret = wland_sdio_rxctl_poll_after_recover(bus, msg, msglen,
						 1200);
		if (ret >= 0)
			return ret;
	}
	...
	if (ret == -ETIMEDOUT &&
	    time_after(jiffies,
		wland_last_rxctl_powercycle_jiffies + msecs_to_jiffies(15000))) {
		wland_last_rxctl_powercycle_jiffies = jiffies;
		WLAND_ERR("rxctl still timeout, power-cycle wifi + short retry\n");
		rda_wifi_power_off();
		msleep(80);
		rda_wifi_power_on();
		msleep(120);
