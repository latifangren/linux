...
static int wland_proto_cdc_cmplt(struct wland_private *drvr, u8 id, u16 len)
{
	...
	do {
		...
		if (prot->msg.wid_msg_id != id)
			WLAND_DBG(DCMD, DEBUG,
				"cdc response id mismatch: got %u expect %u\n",
				prot->msg.wid_msg_id, id);
	} while ((prot->msg.wid_msg_id != id) && --attempts > 0);

	if ((ret >= 0) && (prot->msg.wid_msg_id != id)) {
		WLAND_ERR("cdc response timeout waiting msg id %u (last %u)\n",
			id, prot->msg.wid_msg_id);
		ret = -ETIMEDOUT;
	}
	...
	bool hidden_ssid)
{
	...
	buf[1] = (u8) ((wid & 0xFF00) >> 8);
	buf[2] = size;
	buf[3] = is_p2p ? P2P_GO : ACCESS_POINT;
	buf += (size + 3);
	wid_msg_len += (size + 3);
	...
	buf[1] = (u8) ((wid & 0xFF00) >> 8);
	buf[2] = size;
	buf[3] = DOT11_BSSTYPE_INFRASTRUCTURE;
	buf += (size + 3);
	wid_msg_len += (size + 3);
	...
	wid_msg_len += (size + 3);
	buf += (size + 3);
	WLAND_DBG(DCMD, TRACE, "WID_BCAST_SSID=%u (hidden=%u)\n",
		bcast_ssid, hidden_ssid);
#if 1
	/*
