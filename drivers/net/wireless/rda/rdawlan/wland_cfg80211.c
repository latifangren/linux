...
#ifndef STATION_INFO_INACTIVE_TIME
#define STATION_INFO_INACTIVE_TIME BIT_ULL(NL80211_STA_INFO_INACTIVE_TIME)
#endif

#ifndef STATION_INFO_CONNECTED_TIME
#define STATION_INFO_CONNECTED_TIME BIT_ULL(NL80211_STA_INFO_CONNECTED_TIME)
#endif
...
static __maybe_unused u16 wland_qdbm_to_mw(u8 qdbm)
{
	uint factor = 1;
...
static s32 wland_cfg80211_set_fw_mode(struct wland_if *ifp,
		enum nl80211_iftype dev_role)
{
	...
	ap_mode = (dev_role == NL80211_IFTYPE_AP ||
		dev_role == NL80211_IFTYPE_AP_VLAN ||
		dev_role == NL80211_IFTYPE_P2P_GO);
	p2p_mode = (dev_role == NL80211_IFTYPE_P2P_GO ||
		dev_role == NL80211_IFTYPE_P2P_CLIENT);

	mac_role = ap_mode ? MAC_ROLE_AP : MAC_ROLE_STA;
	err = wland_fil_set_cmd_data(ifp, WID_MAC_ROLE, &mac_role,
		sizeof(mac_role));
	if (err < 0) {
		WLAND_ERR("set WID_MAC_ROLE=%u failed (%d)\n", mac_role, err);
		return err;
	}

	if (ap_mode)
		device_mode = p2p_mode ? P2P_GO : ACCESS_POINT;
	else
		device_mode = p2p_mode ? P2P_CLIENT : BSS_STA;

	err = wland_fil_set_cmd_data(ifp, WID_DEVICE_MODE, &device_mode,
		sizeof(device_mode));
	if (err < 0)
		WLAND_DBG(CFG80211, DEBUG,
			"set WID_DEVICE_MODE=%u failed (%d), continue\n",
			device_mode, err);

	ifp->drvr->dev_mode = ap_mode;
	WLAND_DBG(CFG80211, TRACE,
		"firmware mode switched: role=%u device_mode=%u\n",
		mac_role, device_mode);
	return 0;
}

static s32 wland_wait_scan_idle(struct wland_cfg80211_info *cfg,
	const char *caller, u32 sleep_ms, u32 timeout_ms)
{
	u32 waited = 0;

	while (test_bit(SCAN_STATUS_BUSY, &cfg->scan_status)) {
		if (waited >= timeout_ms) {
			WLAND_ERR("%s: scan busy timeout, force clear (status=%lu)\n",
				caller, cfg->scan_status);
...
static s32 cfg80211_set_wiphy_params(struct wiphy *wiphy, int radio_idx,
	u32 changed)
{
	struct wland_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
...
static void wland_link_down(struct wland_cfg80211_vif *vif)
{
	...
	if (test_bit(VIF_STATUS_CONNECTED, &vif->sme_state)) {
		...
		if (!err) {
			WLAND_DBG(CFG80211, DEBUG, "DISASSOC from AP success\n");
	...
	struct cfg80211_connect_params *sme)
{
	...
	struct wland_bss_info_le *bss, *bss_current = NULL;
	...
	if (dev_role == NL80211_IFTYPE_AP ||
	    dev_role == NL80211_IFTYPE_AP_VLAN ||
	    dev_role == NL80211_IFTYPE_P2P_GO)
		dev_role = NL80211_IFTYPE_STATION;
	else if (dev_role != NL80211_IFTYPE_STATION &&
		 dev_role != NL80211_IFTYPE_P2P_CLIENT)
		dev_role = NL80211_IFTYPE_STATION;

	ifp->vif->wdev.iftype = dev_role;
	ifp->vif->mode = WL_MODE_BSS;
	if (ndev->ieee80211_ptr)
		ndev->ieee80211_ptr->iftype = dev_role;
	clear_bit(VIF_STATUS_AP_CREATING, &ifp->vif->sme_state);
	clear_bit(VIF_STATUS_AP_CREATED, &ifp->vif->sme_state);

	cfg_err = wland_cfg80211_set_fw_mode(ifp, dev_role);
	if (cfg_err < 0) {
		WLAND_ERR("set firmware mode for connect failed (%d)\n", cfg_err);
		return cfg_err;
	}

	if (dev_role == NL80211_IFTYPE_STATION) {
		cfg_err = wland_fil_iovar_data_set(ifp, "set_infra", &infra,
			sizeof(infra));
		if (cfg_err < 0)
			WLAND_ERR("SET_INFRA(sta) error (%d)\n", cfg_err);
	}
	...
	ssid_len = min_t(u8, sme->ssid_len, (u8)IEEE80211_MAX_SSID_LEN);
	if (!sme->bssid || !chan) {
		list_for_each_entry(bss, &cfg->scan_result_list, list) {
			if (bss->SSID_len != ssid_len)
				continue;

			if (memcmp(bss->SSID, sme->ssid, ssid_len))
				continue;

			if (sme->bssid && memcmp(bss->BSSID, sme->bssid, ETH_ALEN))
				continue;
	...
	if (sme->bssid) {
		...
		memcpy(profile->bssid, bss_current->BSSID, ETH_ALEN);
		profile->valid_bssid = true;
	} else {
	...
	WLAND_DBG(CFG80211, TRACE,
		...
		sme->bssid,
		chan ? ieee80211_frequency_to_channel(chan->center_freq) : 0);
	WLAND_DBG(CFG80211, TRACE,
	...
	if (sme->ie && sme->ie_len) {
		...
		if (wpa_ie) {
			sec_ie = (u8 *)wpa_ie;
			sec_ie_len = wpa_ie->len + TLV_HDR_LEN;
		} else {
			rsn_ie = wland_parse_tlvs((u8 *)sme->ie, sme->ie_len,
				WLAN_EID_RSN);
			if (rsn_ie) {
				sec_ie = (u8 *)rsn_ie;
				sec_ie_len = rsn_ie->len + TLV_HDR_LEN;
			}
		}

		if (sec_ie && sec_ie_len) {
			s32 wpaie_err;

			wpaie_err = wland_fil_iovar_data_set(ifp, "wpaie",
				sec_ie, sec_ie_len);
			if (wpaie_err < 0)
				WLAND_DBG(CFG80211, DEBUG,
					"set wpaie failed (%d)\n", wpaie_err);

			err = wland_fil_set_mgmt_ie(ifp, sec_ie, sec_ie_len);
		} else {
			err = wland_fil_set_mgmt_ie(ifp, (u8 *)sme->ie,
				sme->ie_len);
		}
	} else if (bss_current && bss_current->ie && bss_current->ie_length) {
		err = wland_fil_set_mgmt_ie(ifp, (u8 *)bss_current->ie,
			bss_current->ie_length);
	}
	...
	WLAND_DBG(CFG80211, TRACE,
		...
		chan ? chan->center_freq : 0, chanspec);
	 ...
	 */
	if (wland_find_wpsie((u8 *)sme->ie, sme->ie_len) && sme->privacy)
	...
	s32 *dbm)
{
	...
	WLAND_DBG(CFG80211, TRACE, "Enter.\n");
	/*
	 * Keep iw phy/list responsive: report cached tx power without
	 * forcing synchronous firmware round-trips.
	 */
	...
	u8 key_idx, const u8 *mac_addr, struct key_params *params)
{
	...
	enum wland_firmw_wid gtk_wid;
	u16 seq_len;
	s32 err = 0;

	WLAND_DBG(CFG80211, TRACE, "Enter key_idx=%d, params->key_len=%d, params->cipher=0x%x\n", key_idx, params->key_len, params->cipher);
	if (!params->key || params->key_len <= 0 ||
	    params->key_len > WLAN_MAX_KEY_LEN) {
		WLAND_ERR("Invalid GTK length (%d)\n", params->key_len);
		return -EINVAL;
	}
	...
	if (ifp->vif->mode == WL_MODE_AP) {
		if (mac_addr)
			memcpy(gtkey.ea, mac_addr, ETH_ALEN);
		else
			eth_broadcast_addr(gtkey.ea);
		gtk_wid = WID_ADD_TX_GTK;
	} else {
		if (mac_addr && !is_multicast_ether_addr(mac_addr))
			memcpy(gtkey.ea, mac_addr, ETH_ALEN);
		else
			memcpy(gtkey.ea, profile->bssid, ETH_ALEN);
		gtk_wid = WID_ADD_RX_GTK;
	}

	seq_len = min_t(u16, (u16)params->seq_len, (u16)sizeof(gtkey.keyRSC));
	if (params->seq && seq_len)
		memcpy(gtkey.keyRSC, params->seq, seq_len);
	...
	err = wland_fil_set_cmd_data(ifp, gtk_wid, &gtkey,
		MIN(sizeof(gtkey), params->key_len+ETH_ALEN+10));
	if (err < 0 && gtk_wid == WID_ADD_TX_GTK) {
		WLAND_ERR("Set WID_ADD_TX_GTK error (%d), fallback RX_GTK\n", err);
		err = wland_fil_set_cmd_data(ifp, WID_ADD_RX_GTK, &gtkey,
			MIN(sizeof(gtkey), params->key_len + ETH_ALEN + 10));
	}
	if (err < 0) {
		WLAND_ERR("Set GTK key error (%d)\n", err);
		return err;
	}
	...
	struct key_params *params)
{
	...
	if (params->cipher == WLAN_CIPHER_SUITE_AES_CMAC) {
		WLAND_DBG(CFG80211, DEBUG,
			"ignore AES_CMAC mgmt key in AP path\n");
		return 0;
	}
	...
	ap_mode = (ifp->vif->mode == WL_MODE_AP);
	use_ptk = pairwise;

	/*
	 * Pairwise indication can be unreliable in some userspace combinations.
	 * For STA, infer PTK when a unicast peer address is provided.
	 */
	if (!ap_mode && !use_ptk && mac_addr &&
	    !is_multicast_ether_addr(mac_addr))
		use_ptk = true;

	if (use_ptk) {
		const u8 *ptk_addr = mac_addr;

		if (!mac_addr || is_multicast_ether_addr(mac_addr)) {
			if (ap_mode) {
				WLAND_ERR("pairwise key requires unicast station address\n");
				return -EINVAL;
			}

			ptk_addr = ndev_to_prof(ndev)->bssid;
			if (!is_valid_ether_addr(ptk_addr)) {
				WLAND_ERR("pairwise key missing addr and bssid invalid\n");
				return -EINVAL;
			}

			WLAND_DBG(CFG80211, DEBUG,
				"pairwise key fallback to bssid %pM\n", ptk_addr);
		}

		return wland_add_ptkey(wiphy, ndev, key_idx, ptk_addr, params);
	}

	return wland_add_gtkey(wiphy, ndev, key_idx, mac_addr, params);
	...
	struct net_device *ndev, int link_id, u8 key_idx)
{
	...
	if (ifp && ifp->vif &&
	    (ifp->vif->wdev.iftype == NL80211_IFTYPE_AP ||
	     ifp->vif->wdev.iftype == NL80211_IFTYPE_P2P_GO)) {
		WLAND_DBG(CFG80211, DEBUG,
			"ignore default mgmt key idx=%u in AP path\n", key_idx);
		return 0;
	}
...
static s32 wland_cfg80211_get_station(struct wiphy *wiphy,
...
static void ParseNetworkInfo(struct wland_bss_info_le *bi, u8 * pu8MsgBuffer)
{
	...
	struct wland_vs_tlv *wpa_ie, bool is_rsn_ie)
{
	...
	for (i = 0; i < count; i++) {
		...
		case 8: /* SAE, best-effort map for legacy fw */
			WLAND_DBG(CFG80211, TRACE, "RSN_AKM_PSK\n");
			is_rsn_ie ? (wpa_auth |= WPA2_AUTH_PSK) : (wpa_auth |=
				WPA_AUTH_PSK);
			break;
		case 3: /* FT-802.1X */
		case 5: /* 802.1X-SHA256 */
			WLAND_DBG(CFG80211, TRACE,
				"RSN_AKM_UNSPECIFIED(alt)\n");
			is_rsn_ie ? (wpa_auth |=
				WPA2_AUTH_UNSPECIFIED) : (wpa_auth |=
				WPA_AUTH_UNSPECIFIED);
			break;
		default:
			WLAND_DBG(CFG80211, DEBUG,
				"unsupported key mgmt suite type %u\n",
				data[offset]);
		}
		offset++;
	}
...
static s32 cfg80211_parse_ap_settings(struct net_device *ndev,
	struct cfg80211_ap_settings *settings)
{
	s32 err = 0;
	bool pairwise_set = false;
	struct wland_cfg80211_profile *profile = ndev_to_prof(ndev);

	WLAND_DBG(CFG80211, DEBUG, "Enter\n");
	profile->sec.security = NO_ENCRYPT;
	profile->sec.firmware_autype = ANY;
	profile->sec.wpa_auth = 0;
	if (settings->crypto.n_akm_suites > 0)
		profile->sec.wpa_auth = settings->crypto.akm_suites[0];
	...
	if (settings->crypto.cipher_group != NO_ENCRYPT) {
		...
		if (!pairwise_set) {
			if (settings->crypto.cipher_group == WLAN_CIPHER_SUITE_TKIP) {
				...
				pairwise_set = true;
				WLAND_DBG(CFG80211, DEBUG,
					"pairwise missing, fallback TKIP from group cipher\n");
			} else if (settings->crypto.cipher_group ==
				WLAN_CIPHER_SUITE_CCMP ||
				...
				pairwise_set = true;
				WLAND_DBG(CFG80211, DEBUG,
					"pairwise missing, fallback AES from group cipher\n");
			} else {
				profile->sec.security |= AES;
				WLAND_DBG(CFG80211, DEBUG,
					"pairwise missing, fallback AES\n");
			}
		}

		if (!pairwise_set &&
		    (settings->crypto.wpa_versions & NL80211_WPA_VERSION_1 ||
		     settings->crypto.wpa_versions & NL80211_WPA_VERSION_2)) {
			profile->sec.security |= AES;
			WLAND_DBG(CFG80211, DEBUG,
				"pairwise missing, fallback default AES\n");
		}
	} else {
	...
	struct cfg80211_ap_settings *settings)
{
	...
	struct wland_join_params join_params;
	struct wland_tlv *ssid_ie;
	struct wland_tlv *ds_ie;
	struct wland_tlv *rsn_ie;
	struct wland_vs_tlv *wpa_ie;
	enum nl80211_iftype dev_role;
	u32 target_channel = 0;
	...
	u8 pm_mode = NO_POWERSAVE;
	s32 cfg_err;
	s32 ds_offset;
	s32 ie_offset, err = -EPERM;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
	struct ieee80211_channel *ap_chan = NULL;
#endif
	s32 infra = 1;
	...
	if (dev_role != NL80211_IFTYPE_AP &&
	    dev_role != NL80211_IFTYPE_P2P_GO) {
		WLAND_ERR("start_ap with iftype=%d, force AP role\n", dev_role);
		dev_role = NL80211_IFTYPE_AP;
	}

	ifp->vif->wdev.iftype = dev_role;
	ifp->vif->mode = WL_MODE_AP;
	if (ndev->ieee80211_ptr)
		ndev->ieee80211_ptr->iftype = dev_role;
	...
	err = wland_cfg80211_set_fw_mode(ifp, dev_role);
	if (err < 0)
		goto exit;

	memset(&join_params, 0, sizeof(join_params));
	cfg_err = wland_fil_iovar_data_set(ifp, "set_ssid", &join_params,
		sizeof(join_params.ssid_le));
	if (cfg_err < 0)
		WLAND_ERR("clear old ssid failed (%d)\n", cfg_err);

	cfg_err = wland_fil_iovar_data_set(ifp, "set_infra", &infra,
		sizeof(infra));
	if (cfg_err < 0)
		WLAND_ERR("set_infra=1 failed (%d)\n", cfg_err);

	cfg_err = wland_fil_set_cmd_data(ifp, WID_POWER_MANAGEMENT,
		&pm_mode, sizeof(pm_mode));
	if (cfg_err < 0)
		WLAND_ERR("disable power management failed (%d)\n",
			cfg_err);

	cfg_err = wland_fil_set_cmd_data(ifp, WID_POWER_SAVE,
		&pm_mode, sizeof(pm_mode));
	if (cfg_err < 0)
		WLAND_ERR("disable power save failed (%d)\n",
			cfg_err);

	if (test_bit(SCAN_STATUS_BUSY, &cfg->scan_status)) {
		WLAND_DBG(CFG80211, DEBUG,
			"scan busy before start_ap, abort pending scan\n");
		wland_abort_scanning(cfg);
	}

	cfg_err = wland_wait_scan_idle(cfg, "start_ap", 100, 3000);
	if (cfg_err < 0)
		WLAND_ERR("start_ap while scan busy (%d), continue\n", cfg_err);

	/* Force channel explicitly: firmware may not always infer it from beacon template. */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
	...
	if (!target_channel && settings->beacon.head && settings->beacon.head_len) {
		ds_offset = DOT11_MGMT_HDR_LEN + DOT11_BCN_PRB_FIXED_LEN;
		if (settings->beacon.head_len > ds_offset) {
			ds_ie = wland_parse_tlvs((u8 *)&settings->beacon.head[ds_offset],
				settings->beacon.head_len - ds_offset,
				WLAN_EID_DS_PARAMS);
			if (ds_ie && ds_ie->len >= 1)
				target_channel = ds_ie->data[0];
		}
	}

	if (!target_channel && cfg->channel >= 1 && cfg->channel <= 14)
		target_channel = cfg->channel;

	if (target_channel < 1 || target_channel > 14) {
		if (target_channel)
			WLAND_ERR("AP channel %u unsupported, fallback to 6\n",
				target_channel);
		else
			WLAND_ERR("AP channel unresolved, fallback to 6\n");
		target_channel = 6;
	}

	cfg->channel = target_channel;
	cfg_err = wland_fil_iovar_data_set(ifp, "set_channel",
		&target_channel, sizeof(target_channel));
	if (cfg_err < 0) {
		ap_channel_u8 = (u8)target_channel;
		cfg_err = wland_fil_set_cmd_data(ifp, WID_CURRENT_CHANNEL,
			&ap_channel_u8, sizeof(ap_channel_u8));
	}
	if (cfg_err < 0) {
		WLAND_ERR("set AP channel %u failed (%d)\n",
			target_channel, cfg_err);
	}
	WLAND_DBG(CFG80211, TRACE, "start_ap target_channel=%u\n",
		target_channel);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
	ap_chan = ieee80211_get_channel(wiphy,
		ieee80211_channel_to_frequency(target_channel,
			NL80211_BAND_2GHZ));

	if (ap_chan)
		ap_chanspec = channel_to_chanspec(&cfg->d11inf, ap_chan);
	else
		WLAND_ERR("AP channel object unavailable for ch=%u\n",
			target_channel);
#endif
	 ...
	 */
	profile->beacon = settings->beacon_interval ? settings->beacon_interval : 100;
	profile->dtim = settings->dtim_period ? settings->dtim_period : 1;

	if (!settings->beacon_interval)
		WLAND_DBG(CFG80211, DEBUG,
			"beacon interval unset, fallback to %u\n", profile->beacon);
	if (!settings->dtim_period)
		WLAND_DBG(CFG80211, DEBUG,
			"dtim period unset, fallback to %u\n", profile->dtim);

	beacon_interval = (u16)profile->beacon;
	cfg_err = wland_fil_set_cmd_data(ifp, WID_BEACON_INTERVAL,
		&beacon_interval, sizeof(beacon_interval));
	if (cfg_err < 0)
		WLAND_ERR("set beacon interval %u failed (%d)\n",
			profile->beacon, cfg_err);

	dtim_period = (u8)profile->dtim;
	cfg_err = wland_fil_set_cmd_data(ifp, WID_DTIM_PERIOD,
		&dtim_period, sizeof(dtim_period));
	if (cfg_err < 0)
		WLAND_ERR("set dtim period %u failed (%d)\n",
			profile->dtim, cfg_err);
	...
	cfg_err = wland_fil_set_cmd_data(ifp, WID_BCAST_SSID,
		&bcast_ssid, sizeof(bcast_ssid));
	if (cfg_err < 0)
		WLAND_ERR("pre-start BCAST_SSID=%u failed (%d)\n",
			bcast_ssid, cfg_err);
	...
	if (!err && profile->ssid.SSID_len) {
		struct wland_fil_bss_enable_le bss_enable;

		memset(&join_params, 0, sizeof(join_params));
		memcpy(join_params.ssid_le.SSID, profile->ssid.SSID,
			profile->ssid.SSID_len);
		join_params.ssid_le.SSID_len = cpu_to_le32(profile->ssid.SSID_len);
		join_params_size = sizeof(join_params.ssid_le);
		...
		cfg_err = wland_fil_iovar_data_set(ifp, "set_ssid", &join_params,
			join_params_size);
		if (cfg_err < 0 && ap_chanspec) {
			memset(join_params.params_le.bssid, 0xFF,
				sizeof(join_params.params_le.bssid));
			join_params.params_le.chanspec_num = cpu_to_le32(1);
			join_params.params_le.chanspec_list[0] =
				cpu_to_le16(ap_chanspec);
			join_params_size = sizeof(join_params.ssid_le) +
				sizeof(join_params.params_le);
			WLAND_DBG(CFG80211, TRACE,
				"AP set_ssid retry with chanspec 0x%04x\n",
				ap_chanspec);
			cfg_err = wland_fil_iovar_data_set(ifp, "set_ssid",
				&join_params, join_params_size);
		}

		if (cfg_err < 0)
			WLAND_ERR("AP set_ssid activation failed (%d)\n", cfg_err);
		else
			ap_activated = true;

		bss_enable.bsscfg_idx = cpu_to_le32(ifp->bssidx);
		bss_enable.enable = cpu_to_le32(1);
		cfg_err = wland_fil_iovar_data_set(ifp, "bss", &bss_enable,
			sizeof(bss_enable));
		if (cfg_err < 0)
			WLAND_ERR("AP bss enable failed (%d)\n", cfg_err);
		else
			ap_activated = true;

		if (!ap_activated) {
			WLAND_ERR("AP activation failed: set_ssid and bss both failed\n");
			err = -EIO;
		}
	}
	...
	cfg_err = wland_fil_set_cmd_data(ifp, WID_BCAST_SSID,
		&bcast_ssid, sizeof(bcast_ssid));
	if (cfg_err < 0)
		WLAND_ERR("post-start BCAST_SSID=%u failed (%d)\n",
			bcast_ssid, cfg_err);
...
{
	struct wland_if *ifp = netdev_priv(ndev);
	enum nl80211_iftype dev_role = NL80211_IFTYPE_STATION;
	s32 err = 0;
	s32 cfg_err;
	s32 infra = 1;
	...
	cfg_err = wland_cfg80211_set_fw_mode(ifp, dev_role);
	if (cfg_err < 0)
		WLAND_ERR("switch STA mode after stop_ap failed:%d\n", cfg_err);

	cfg_err = wland_fil_iovar_data_set(ifp, "set_infra", &infra,
		sizeof(infra));
	if (cfg_err < 0)
		WLAND_ERR("SET_INFRA(sta) after stop_ap failed:%d\n", cfg_err);
	...
	struct cfg80211_ap_update *info)
{
	 ...
	 */
	err = wland_vif_set_mgmt_ie(ifp->vif, WLAND_VNDR_IE_PRBRSP_FLAG,
		info->beacon.proberesp_ies, info->beacon.proberesp_ies_len);
	if (err)
		WLAND_ERR("Set Probe Resp IE Failed\n");
	else
		WLAND_DBG(CFG80211, TRACE, "Applied Vndr IEs for Probe Resp\n");
	...
	struct station_del_parameters *params)
{
	...
	if (mac)
		...
		WLAND_DBG(CFG80211, TRACE, "Enter(<broadcast>)\n");
	...
	if (!mac) {
		/* cfg80211 may pass NULL to request deleting all stations. */
		eth_broadcast_addr(scbval.ea);
		WLAND_DBG(CFG80211, DEBUG,
			"del_station with NULL mac, use broadcast\n");
	} else {
...
static void __maybe_unused cfg80211_mgmt_frame_register(struct wiphy *wiphy,
#if  LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
	struct wireless_dev *wdev,
#else				/*LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0) */
...
static s32 cfg80211_mgmt_tx(struct wiphy *wiphy,
	...
	u64 *cookie)
#else				/*LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0) */
	struct net_device *ndev, struct ieee80211_channel *channel, bool offchan,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
static void cfg80211_reg_notifier(struct wiphy *wiphy,
	struct regulatory_request *request)
{
	...
	if (request->initiator != NL80211_REGDOM_SET_BY_USER) {
		WLAND_DBG(CFG80211, DEBUG,
			"Ignore reg_notifier initiator:%d\n",
			request->initiator);
		return;
	}
...
#else
static int cfg80211_reg_notifier(struct wiphy *wiphy,
	struct regulatory_request *request)
{
	struct wland_cfg80211_info *cfg =
		(struct wland_cfg80211_info *) wiphy_priv(wiphy);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)
	/*
		 * Keep AP_SME disabled for this fullmac path. Advertising it can
		 * trigger AP activation/auth timeout with recent userspace.
	 */
...
static void wland_log_conn_status(const char *state,
	struct wland_cfg80211_profile *profile)
{
	char ssid[sizeof(profile->ssid.SSID) + 1];
	u32 ssid_len = min_t(u32, profile->ssid.SSID_len,
				    (u32)sizeof(profile->ssid.SSID));
	...
	memcpy(ssid, profile->ssid.SSID, ssid_len);
	ssid[ssid_len] = '\0';
	pr_info("### %s %s\n", state, ssid);
}
	...
	bool completed)
{
	...
	if (test_and_clear_bit(VIF_STATUS_CONNECTING, &ifp->vif->sme_state)) {
		if (completed) {
			...
			wland_log_conn_status("Connected to", profile);
			conn_info->connected_jiffies = jiffies;
		} else {
			wland_log_conn_status("Failed to connect", profile);
			conn_info->connected_jiffies = 0;
		}
	...
	const struct wland_event_msg *e, void *data)
{
	...
	if (!check_vif_up(ifp->vif)) {
		WLAND_DBG(CFG80211, DEBUG,
			"Ignore event before vif setup (event:%d,reason:%d)\n",
	...
	if (ifp->vif->mode == WL_MODE_AP) {
		...
		&& (e->status == STATUS_SUCCESS)) {
		if (e->action == EVENT_AUTH_IND ||
		    e->action == EVENT_ASSOC_IND ||
		    e->action == EVENT_REASSOC_IND) {
			WLAND_DBG(CFG80211, DEBUG,
				"ignore interim STA connect event action=%u\n",
				e->action);
			goto fail;
		}
		...
		if (ifp->vif->mode == WL_MODE_IBSS) {
			...
			cfg80211_ibss_joined(ndev, e->addr, NULL, GFP_KERNEL);
			clear_bit(VIF_STATUS_CONNECTING, &ifp->vif->sme_state);
			set_bit(VIF_STATUS_CONNECTED, &ifp->vif->sme_state);
		} else {
		...
		if (ifp->vif->mode != WL_MODE_IBSS &&
		    !cfg->in_disconnecting &&
		    e->action != 0 &&
		    e->action != EVENT_DEAUTH_IND &&
		    e->action != EVENT_DISASSOC_IND &&
		    test_bit(VIF_STATUS_CONNECTED, &ifp->vif->sme_state) &&
		    conn_info->connected_jiffies &&
		    time_before(jiffies,
			conn_info->connected_jiffies +
			msecs_to_jiffies(CONNECT_DISCONNECT_GUARD_MS))) {
			WLAND_DBG(CFG80211, DEBUG,
				"ignore generic disconnect action=%u %ums after connect\n",
				e->action,
		...
		if (ifp->vif->mode != WL_MODE_IBSS) {
			...
			if (test_and_clear_bit(VIF_STATUS_CONNECTED,
					&ifp->vif->sme_state)) {
				conn_info->connected_jiffies = 0;
				wland_log_conn_status("Disconnected to", profile);
		...
		if (ifp->vif->mode != WL_MODE_IBSS) {
			if (test_and_clear_bit(VIF_STATUS_CONNECTING,
					&ifp->vif->sme_state))
				wland_log_conn_status("Failed to connect", profile);
		} else {
			wland_bss_connect_done(cfg, ndev, e, false);
...
static __maybe_unused s32 wland_dongle_roam(struct wland_if *ifp, u32 roamvar, u32 bcn_timeout)
{
	s32 err = 0;
...
static __maybe_unused s32 wland_update_wiphybands(struct wland_cfg80211_info *cfg, bool notify)
{
	struct wland_if *ifp = netdev_priv(cfg_to_ndev(cfg));
	struct wiphy *wiphy = cfg_to_wiphy(cfg);
...
s32 wland_cfg80211_up(struct net_device * ndev)
{
	...
	if (cfg->dongle_up) {
		WLAND_DBG(CFG80211, TRACE, "dongle already up\n");
		goto up_exit;
	}

	/*
	 * Keep ndo_open fast: it runs under rtnl_lock and must not block on
	 * firmware command paths. Runtime policy programming is handled by
	 * dedicated cfg80211 operations later.
	 */
