...
int wland_sdio_send_pkt(struct wland_sdio *bus, struct sk_buff *pkt, uint count)
{
	...
	if ((ret = wland_sdio_flow_ctrl(bus->sdiodev))) {
		/*
		 * Some 91x firmware builds intermittently miss RXCMPL before control
		 * requests. Allow TXCTL frames to continue so command transport does
		 * not fail permanently with -EIO.
		 */
		if (bus->ctrl_frame_stat) {
			WLAND_DBG(SDIO, DEBUG,
				"wland_sdio_flow_ctrl failed for ctrl frame (%d), continue\n",
				ret);
		} else {
