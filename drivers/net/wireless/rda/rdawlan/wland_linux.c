...
static netdev_tx_t wland_netdev_start_xmit(struct sk_buff *skb,
	...
	char *name, u8 * mac_addr)
{
	...
	if (!drvr->p2p_enable && bssidx == 1) {
		 ...
		 */
		ndev = alloc_netdev(sizeof(struct wland_if), name,
...
/* msec: allow slower SDIO probe/firmware patch sequence to finish */
...
static int wlanfmac_module_init(void)
{
	 ...
	 */
	waitret = down_timeout(&registration_sem,
			msecs_to_jiffies(REGISTRATION_TIMEOUT));
	if (waitret != 0) {
		WLAND_ERR("sdio_register_driver still probing, continue asynchronously\n");
		return 0;
	}

	if (!registration_check) {
		WLAND_ERR("sdio_register_driver returned error\n");
