...
static inline void wland_timer_list_cb(struct timer_list *tl)
{
	struct wland_drv_timer *timer = timer_container_of(timer, tl, tl);

	wland_timer_handler((ulong)timer);
}
...
static inline void wland_init_timer(struct wland_drv_timer *timer, timer_cb_fn_t func, void *data, u16 event)
{
	...
	timer_setup(&timer->tl, wland_timer_list_cb, 0);
...
extern void wland_pkt_align(struct sk_buff *p, int len, int align);

extern void get_ssid(u8*data, u8*ssid, u8*p_ssid_len);
extern u16  get_cap_info(u8*data);
extern void get_ssid(u8* data, u8* ssid, u8* p_ssid_len);
extern void get_BSSID(u8* data, u8* bssid);
extern u8   get_type(u8 *header);
extern u8   get_sub_type(u8 *header);
extern u8   get_to_ds(u8 *header);
extern u8   get_from_ds(u8 *header);
extern void get_address1(u8 *pu8msa, u8 *addr);
extern void get_address2(u8 *pu8msa, u8 *addr);
extern void get_address3(u8 *pu8msa, u8 *addr);
extern u16  get_assoc_resp_cap_info(u8 *data);
extern u16  get_asoc_status(u8 *data);
extern u16  get_asoc_id(u8 *data);
