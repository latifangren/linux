...
static int power_on(int isWifi)
{
	...
	if (ret)
		goto power_off;
	pr_debug("%s write soft_reset succeed!!\n", __func__);
...
int rda_5991g_wifi_power_on(void)
{
	...
	if (ret)
		goto power_off;
	pr_debug("%s write control_mode_disable succeed!!\n", __func__);
	if (check_test_mode()) {
	...
	rda_combo_i2c_unlock();
	msleep(100);
	/* Keep 26MHz reference alive while WiFi is on; SDIO can depend on it. */
