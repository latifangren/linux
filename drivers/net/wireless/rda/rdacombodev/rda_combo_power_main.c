...
#ifndef RDA_CLK_OUT
#define RDA_CLK_OUT "clk_out"
#endif

#ifndef RDA_CLK_AUX
#define RDA_CLK_AUX "aux_clk"
#endif

#ifndef GPIO_BT_HOST_WAKE
#define GPIO_BT_HOST_WAKE (-1)
#endif
...
static bool combo_rfkill_ready;
static bool rda_wifi_bootstrap_queued;
...
static bool clk32k_needs_hwctrl;
static bool clk26m_needs_hwctrl;
static HWP_SYS_CTRL_T *hwp_mdSysCtrl;
static HWP_CFG_REGS_T *hwp_cfgRegs;
static int combo_wifi_en_gpio = -1;
static bool combo_wifi_en_active_low;
static bool combo_wifi_en_inited;
static bool combo_5991g_ctrl_mode_disabled;

static struct clk *rda_combo_get_dt_clock(const char *clock_name)
{
	struct device_node *np;
	struct clk *clk;

	np = of_find_compatible_node(NULL, NULL, "rda,8810pl-combo-net");
	if (!np)
		return ERR_PTR(-ENODEV);

	clk = of_clk_get_by_name(np, clock_name);
	of_node_put(np);
...
static void rda_combo_wifi_enable_init(void)
{
	struct device_node *np;
	int gpio;
	int level;

	if (combo_wifi_en_inited)
		return;
	combo_wifi_en_inited = true;

	np = of_find_compatible_node(NULL, NULL, "rda,8810pl-combo-net");
	if (!np) {
		pr_info("rda_combo: combo-net DT node not found, skip wifi-enable gpio\n");
		return;
	}

	gpio = of_get_named_gpio(np, "wifi-enable-gpios", 0);
	combo_wifi_en_active_low = of_property_read_bool(np,
						       "wifi-enable-active-low");
	of_node_put(np);

	if (!gpio_is_valid(gpio)) {
		pr_debug("rda_combo: wifi-enable-gpios is not defined/valid\n");
		return;
	}

	if (gpio_request(gpio, "rda_wifi_enable")) {
		pr_warn("rda_combo: failed to request wifi-enable gpio %d\n", gpio);
		return;
	}

	combo_wifi_en_gpio = gpio;

	/* Default to enabled state; this line powers SDIO visibility on some boards. */
	level = combo_wifi_en_active_low ? 0 : 1;
	gpio_direction_output(combo_wifi_en_gpio, level);
	gpio_set_value(combo_wifi_en_gpio, level);

	pr_info("rda_combo: wifi-enable gpio=%d active_low=%d level=%d\n",
		combo_wifi_en_gpio, combo_wifi_en_active_low ? 1 : 0, level);
}

static void rda_combo_wifi_enable_assert(void)
{
	int level;

	rda_combo_wifi_enable_init();
	if (!gpio_is_valid(combo_wifi_en_gpio))
		return;

	level = combo_wifi_en_active_low ? 0 : 1;
	gpio_direction_output(combo_wifi_en_gpio, level);
	gpio_set_value(combo_wifi_en_gpio, level);
	pr_info("rda_combo: wifi-enable assert gpio=%d level=%d active_low=%d\n",
		combo_wifi_en_gpio, level, combo_wifi_en_active_low ? 1 : 0);
}

static void rda_combo_wifi_enable_deassert(void)
{
	int level;
	...
	level = combo_wifi_en_active_low ? 1 : 0;
	gpio_direction_output(combo_wifi_en_gpio, level);
	gpio_set_value(combo_wifi_en_gpio, level);
	pr_info("rda_combo: wifi-enable deassert gpio=%d level=%d active_low=%d\n",
		combo_wifi_en_gpio, level, combo_wifi_en_active_low ? 1 : 0);
}

static void rda_combo_wifi_enable_pulse(unsigned int off_ms,
					unsigned int on_ms)
{
	int off_level;
	int on_level;
	...
	on_level = combo_wifi_en_active_low ? 0 : 1;
	off_level = combo_wifi_en_active_low ? 1 : 0;

	gpio_direction_output(combo_wifi_en_gpio, off_level);
	gpio_set_value(combo_wifi_en_gpio, off_level);
	msleep(off_ms);

	gpio_direction_output(combo_wifi_en_gpio, on_level);
	gpio_set_value(combo_wifi_en_gpio, on_level);
	pr_info("rda_combo: wifi-enable pulse gpio=%d off=%d on=%d active_low=%d\n",
		combo_wifi_en_gpio, off_level, on_level,
		combo_wifi_en_active_low ? 1 : 0);
	msleep(on_ms);
}

static bool rda_combo_try_5991g_control_mode_disable(void)
{
	int ret;

	if (combo_5991g_ctrl_mode_disabled)
		return true;

	if (wlan_version != WLAN_VERSION_91_G || !rda_wifi_rf_client)
		return false;

	rda_combo_i2c_lock();
	ret = i2c_write_1_addr_2_data(rda_wifi_rf_client, 0x3f, 0x0000);
	if (!ret)
		ret = i2c_write_1_addr_2_data(rda_wifi_rf_client, 0x30, 0x0141);
	rda_combo_i2c_unlock();

	if (ret) {
		pr_warn("rda_combo: 5991g control_mode_disable fallback failed: %d\n",
			ret);
		return false;
	}

	combo_5991g_ctrl_mode_disabled = true;
	pr_warn("rda_combo: applied 5991g control_mode_disable fallback\n");

	return true;
}

static void rda_combo_hwclk_map_resources(void)
{
	if (!hwp_mdSysCtrl)
		hwp_mdSysCtrl = (HWP_SYS_CTRL_T *)ioremap(RDA_MD_SYSCTRL_PHYS,
						  RDA_MD_SYSCTRL_SIZE);

	if (!hwp_cfgRegs)
		hwp_cfgRegs = (HWP_CFG_REGS_T *)ioremap(RDA_CONFIG_REGS_PHYS,
					     RDA_CONFIG_REGS_SIZE);

	if (!hwp_mdSysCtrl || !hwp_cfgRegs)
		pr_warn_once("rda_combo: failed to map hw clock control registers\n");
}

static void rda_combo_hwclk_32k_enable(void)
{
	u32 val;

	if (!clk32k_needs_hwctrl)
		return;

	rda_combo_hwclk_map_resources();
	if (!hwp_mdSysCtrl || !hwp_cfgRegs)
		return;

	hwp_mdSysCtrl->REG_DBG = SYS_CTRL_PROTECT_UNLOCK;

	/* AP_PIN_CLK_OUT must stay on alternate function for clock output. */
	val = hwp_cfgRegs->AP_GPIO_A_Mode;
	val &= ~(1U << AP_PIN_CLK_OUT);
	hwp_cfgRegs->AP_GPIO_A_Mode = val;

	/* Route CLK_OUT function to output mux. */
	val = hwp_cfgRegs->Alt_mux_select;
	val = (val & ~CFG_REGS_CLK_OUT_MASK) | CFG_REGS_CLK_OUT_CLK_OUT;
	hwp_cfgRegs->Alt_mux_select = val;

	/* Use OSC source on clk_out path. */
	val = hwp_mdSysCtrl->Cfg_Clk_Out;
	val &= ~((31U << 0) | (3U << 8));
	val |= SYS_CTRL_CLKOUT_DIVIDER(0) | SYS_CTRL_CLKOUT_SEL_OSC;
	hwp_mdSysCtrl->Cfg_Clk_Out = val;

	hwp_mdSysCtrl->Clk_Other_Enable = SYS_CTRL_ENABLE_OC_CLK_OUT;
	hwp_mdSysCtrl->REG_DBG = SYS_CTRL_PROTECT_LOCK;
}

static void rda_combo_hwclk_32k_disable(void)
{
	if (!clk32k_needs_hwctrl || !hwp_mdSysCtrl)
		return;

	hwp_mdSysCtrl->REG_DBG = SYS_CTRL_PROTECT_UNLOCK;
	hwp_mdSysCtrl->Clk_Other_Disable = SYS_CTRL_DISABLE_OC_CLK_OUT;
	hwp_mdSysCtrl->REG_DBG = SYS_CTRL_PROTECT_LOCK;
}

static void rda_combo_hwclk_26m_enable(void)
{
	if (!clk26m_needs_hwctrl)
		return;
	...
	hwp_mdSysCtrl->REG_DBG = SYS_CTRL_PROTECT_UNLOCK;
	hwp_mdSysCtrl->Cfg_Clk_Auxclk = SYS_CTRL_AUXCLK_EN_ENABLE;
	hwp_mdSysCtrl->REG_DBG = SYS_CTRL_PROTECT_LOCK;
}

static void rda_combo_hwclk_26m_disable(void)
{
	if (!clk26m_needs_hwctrl || !hwp_mdSysCtrl)
		return;

	hwp_mdSysCtrl->REG_DBG = SYS_CTRL_PROTECT_UNLOCK;
	hwp_mdSysCtrl->Cfg_Clk_Auxclk = SYS_CTRL_AUXCLK_EN_DISABLE;
	hwp_mdSysCtrl->REG_DBG = SYS_CTRL_PROTECT_LOCK;
}
...
{
	if (IS_ERR_OR_NULL(clk32k) && !clk32k_needs_hwctrl)
		return;

	if (clock_mask_32k & CLOCK_MASK_ALL) {

	} else {
		if (!IS_ERR_OR_NULL(clk32k))
			clk_prepare_enable(clk32k);
		rda_combo_hwclk_32k_enable();
	}
	clock_mask_32k |= mask;
}
...
{
	if (IS_ERR_OR_NULL(clk26m) && !clk26m_needs_hwctrl)
		return;

	if (clock_mask_26m & CLOCK_MASK_ALL) {

	} else {
		if (!IS_ERR_OR_NULL(clk26m))
			clk_prepare_enable(clk26m);
		rda_combo_hwclk_26m_enable();
	}
	clock_mask_26m |= mask;
}
...
static void wlan_read_version_from_chip(void)
{
	...
	if (wlan_version == 0)
		...
		pr_debug("read project_id:%x version:%x wlan_version:%x\n",
			 project_id, chip_version, wlan_version);
err:
	ret = i2c_write_1_addr_2_data(rda_wifi_rf_client, 0x3f, 0x0000);
	return;
...
static void rda_wifi_refresh_sdio(bool wlan_on)
{
	int i;
	int tries = wlan_on ? 20 : 1;
	bool attached = false;
	u8 present = wlan_on ? 1 : 0;

	rda_mmc_set_present(1, present);
	rda_mmc_set_sdio_irq(1, false);
	if (wlan_on)
		rda_combo_wifi_enable_assert();

	for (i = 0; i < tries; i++) {
		int wait_ms;

		if (wlan_on && !combo_5991g_ctrl_mode_disabled && i == 5)
			rda_combo_try_5991g_control_mode_disable();

		if (wlan_on && rda_mmc_card_attached(1)) {
			pr_debug("rda_wifi_refresh_sdio: mmc1 already attached before attempt %d\n",
				 i + 1);
			break;
		}

		pr_debug("rda_wifi_refresh_sdio: mmc1 scan attempt %d/%d\n",
			 i + 1, tries);
		...
		attached = false;
		if (wlan_on) {
			/* mmc_detect_change is asynchronous, so poll briefly before rescanning. */
			for (wait_ms = 0; wait_ms < 250; wait_ms += 25) {
				if (rda_mmc_card_attached(1)) {
					attached = true;
					break;
				}
				msleep(25);
			...
			if (attached) {
				pr_debug("rda_wifi_refresh_sdio: mmc1 attached after attempt %d\n",
					 i + 1);
				break;
			}
		}
...
static void rda_wifi_bootstrap_workfn(struct work_struct *work)
{
	int ret;
	...
	/*
	 * Run one bootstrap pass so SDIO enumeration does not depend on
	 * userspace/module ordering.
	 */
	ret = rda_wifi_power_on();
	if (ret)
		pr_warn("rda_wifi_rf: bootstrap rda_wifi_power_on failed: %d\n", ret);
}

static DECLARE_DELAYED_WORK(rda_wifi_bootstrap_work, rda_wifi_bootstrap_workfn);
...
static int rda_wifi_rf_probe(struct i2c_client *client)
{
	...
	if (!rda_wifi_bootstrap_queued) {
		rda_wifi_bootstrap_queued = true;
		schedule_delayed_work(&rda_wifi_bootstrap_work,
...
{
	cancel_delayed_work_sync(&rda_wifi_bootstrap_work);
	rda_wifi_bootstrap_queued = false;
	rda_wifi_rf_client = NULL;
}
...
int rda_wifi_power_off(void)
{
	...
	rda_wifi_refresh_sdio(false);
	rda_combo_wifi_enable_deassert();
...
int rda_wifi_power_on(void)
{
	...
	combo_5991g_ctrl_mode_disabled = false;

	/* Keep optional board WiFi-enable line asserted before chip power sequence. */
	rda_combo_wifi_enable_assert();
	/* Some modules require an explicit EN low->high edge before SDIO responds. */
	rda_combo_wifi_enable_pulse(20, 40);
	...
	if (!ret && wlan_version == WLAN_VERSION_91_G)
		combo_5991g_ctrl_mode_disabled = true;

	if (!ret)
		rda_wifi_refresh_sdio(true);
	else {
		pr_warn("rda_wifi_power_on: chip power script failed (%d), keep SDIO scan fallback active\n",
			ret);
		rda_wifi_refresh_sdio(true);
	}
...
static void rda_combo_sleep_worker_task(struct work_struct *work)
{
...
{
	if (!combo_rfkill_ready)
		return 0;

	pr_debug("wlan_rfkill_set %d\n", blocked);
	if (blocked) {
...
#endif
static int rda_combo_power_ctrl_init(void)
{
	int ret = 0;

	pr_debug("rda_combo_power_ctrl_init begin\n");
	rda_combo_wifi_enable_init();
	...
	rda_combo_class = class_create("rda_combo");
	...
	if (gpio_is_valid(GPIO_BT_HOST_WAKE)) {
		...
		pr_debug("Skip BT host wake irq: GPIO_BT_HOST_WAKE is invalid\n");
	}

	clk32k = rda_combo_get_dt_clock("rda_out");
	if (IS_ERR(clk32k))
		clk32k = clk_get(NULL, RDA_CLK_OUT);
	if (IS_ERR(clk32k)) {
		pr_warn("rda_combo: cannot get clock clk32k; using pure hw fallback\n");
		clk32k = NULL;
		clk32k_needs_hwctrl = true;
	} else {
		clk32k_needs_hwctrl = false;
	}

	clk26m = rda_combo_get_dt_clock("rda_aux");
	if (IS_ERR(clk26m))
		clk26m = clk_get(NULL, RDA_CLK_AUX);
	if (IS_ERR(clk26m)) {
		pr_warn("rda_combo: cannot get clock clk26m; using pure hw fallback\n");
		clk26m = NULL;
		clk26m_needs_hwctrl = true;
	} else {
		clk26m_needs_hwctrl = false;
	}

	/* Keep direct hw clock control enabled in parallel with clk API. */
	clk32k_needs_hwctrl = true;
	clk26m_needs_hwctrl = true;
	...
	combo_rfkill_ready = true;

	pr_debug("rda_combo_power_ctrl_init end\n");
	return 0;
...
static void rda_combo_power_ctrl_exit(void)
{
	cancel_delayed_work_sync(&rda_wifi_bootstrap_work);
	rda_wifi_bootstrap_queued = false;
	i2c_del_driver(&rda_wifi_core_driver);
