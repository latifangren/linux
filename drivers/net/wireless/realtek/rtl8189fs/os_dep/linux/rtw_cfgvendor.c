...
static struct wiphy_vendor_command rtw_vendor_cmds[] = {
#if defined(GSCAN_SUPPORT) && 0
	{
		{
...
int rtw_cfgvendor_attach(struct wiphy *wiphy)
{
	...
	for (i = 0; i < ARRAY_SIZE(rtw_vendor_cmds); i++) {
		if (!rtw_vendor_cmds[i].policy) {
			rtw_vendor_cmds[i].policy = VENDOR_CMD_RAW_DATA;
			rtw_vendor_cmds[i].maxattr = 0;
		}
	}
