...
)
{
	...
	if (ndev) {
		...
		} else if (adapter == dvobj_get_primary_adapter(dvobj)) {
			RTW_WARN(FUNC_NDEV_FMT" skip deleting primary ndev\n", FUNC_NDEV_ARG(ndev));
			ret = -EBUSY;
		} else {
		...
		struct cfg80211_ap_update *info)
{
	...
	ret = rtw_add_beacon(adapter,
		info->beacon.head, info->beacon.head_len,
...
static void __maybe_unused cfg80211_rtw_mgmt_frame_register(struct wiphy *wiphy,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0))
	struct wireless_dev *wdev,
#else
	struct net_device *ndev,
...
static void rtw_cfgvendor_sanitize_wiphy(struct wiphy *wiphy)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0)) || defined(RTW_VENDOR_EXT_SUPPORT)
	int i;

	if (!wiphy->vendor_commands || !wiphy->n_vendor_commands)
		return;

	for (i = 0; i < wiphy->n_vendor_commands; i++) {
		const struct wiphy_vendor_command *vcmd = &wiphy->vendor_commands[i];

		if (vcmd->policy && (vcmd->doit || vcmd->dumpit))
			continue;

		RTW_WARN(FUNC_WIPHY_FMT" disable vendor cmds (idx:%d policy:%p doit:%p dumpit:%p)\n",
			FUNC_WIPHY_ARG(wiphy), i, vcmd->policy, vcmd->doit, vcmd->dumpit);
...
int rtw_wiphy_register(struct wiphy *wiphy)
{
	...
	rtw_cfgvendor_sanitize_wiphy(wiphy);
#endif
...
void rtw_wdev_unregister(struct wireless_dev *wdev)
{
	...
	#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 11, 0)) || defined(COMPAT_KERNEL_RELEASE)
	if (wdev->connected) {
		RTW_INFO(FUNC_ADPT_FMT" clear connected state by cfg80211_disconnected\n", FUNC_ADPT_ARG(adapter));
	...
	if (pwdev_priv->pmon_ndev) {
		u8 rtnl_lock_needed;
		struct net_device *mon_ndev = pwdev_priv->pmon_ndev;
		struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
		...
		rtnl_lock_needed = rtw_rtnl_lock_needed(dvobj);
		if (rtnl_lock_needed)
			unregister_netdev(mon_ndev);
		else
			unregister_netdevice(mon_ndev);
	}
}
