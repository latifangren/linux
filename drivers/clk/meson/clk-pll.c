...
static int __meson_secure_clk_pll_set_rate(struct clk_hw *hw,
					   unsigned long rate,
					   unsigned long parent_rate)
{
	...
	if (!strcmp(clk_hw_get_name(hw), "sys_pll_dco")) {
		arm_smccc_smc(CLK_SECURE_RW, SYS_PLL_STEP0, rate,
			      0, 0, 0, 0, 0, &res);
	} else if (!strcmp(clk_hw_get_name(hw), "gp1_pll_dco")) {
		arm_smccc_smc(CLK_SECURE_RW, GP1_PLL_STEP0, rate,
			      0, 0, 0, 0, 0, &res);
	} else {
	...
	/* Wait for firmware to apply step 0 writes. */
	udelay(10);
...
static int meson_secure_clk_pll_set_rate(struct clk_hw *hw,
					 unsigned long rate,
					 unsigned long parent_rate)
{
	...
	if (ret == -ETIMEDOUT) {
		pr_info("%s: %s did not lock, retrying rate %lu\n",
...
static int meson_secure_clk_pll_enable(struct clk_hw *hw)
{
	...
	return meson_secure_clk_pll_set_rate(hw, rate,
...
/* Backward-compatible secure PLL ops name used by vendor SC2 clock data. */
...
EXPORT_SYMBOL_NS_GPL(meson_secure_pll_v2_ops, "CLK_MESON");
