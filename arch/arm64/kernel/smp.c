...
static void __iomem *rtk_secondary_marks;
...
int __cpu_up(unsigned int cpu, struct task_struct *idle)
{
	...
	if (!rtk_secondary_marks)
		rtk_secondary_marks = ioremap(0x98007f40, 0x10);
	if (rtk_secondary_marks)
		pr_crit("CPU%u: secondary marks entry 0x%08x init 0x%08x startup 0x%08x\n",
			cpu, readl(rtk_secondary_marks),
			readl(rtk_secondary_marks + 0x4),
			readl(rtk_secondary_marks + 0x8));
	else
		pr_crit("CPU%u: failed to map secondary marks\n", cpu);
